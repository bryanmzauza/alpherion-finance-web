...
| `GET /v1/market/events/calendar?from=&to=&kind=&type=&category=` | Contagem de eventos por dia e tipo — a vista mensal de `/agenda` (o mês inteiro passa do teto de 500 itens de `/events`). `kind` e `category` aceitam vários valores; janela de até 42 dias nas duas rotas | v1.0 |
...
| **B3 — cadastro e eventos corporativos** | Tickers e classe de cada papel; setor/subsetor/segmento; CNPJ e código CVM do emissor; proventos, desdobramentos, bonificações | **Cadastro de instrumentos** (`InstrumentsConsolidatedFile`, CSV diário em `arquivos.b3.com.br`) — ticker, categoria B3 (`SHARES`, `UNIT`, `BDR`, `ETF…`, `FUNDS`), ISIN; **classificação setorial** (planilha de `GetDownloadIndustryClassification`); **emissores** (`GetInitialCompanies`, JSON) — CNPJ e código CVM. Eventos: endpoints do site de listagem | Diária | `securities` (cadastro), `sectors`, `corporate_actions` | Classe pela **categoria da B3**, nunca pelo sufixo do ticker. Segmento de FII vem do informe mensal da CVM (`Segmento_Atuacao`), casado pelo ISIN. Endpoints JSON **não documentados**; podem mudar sem aviso. Registrar em `data_sources` (revisado em 23/09/2026) |
...
| **BCB SGS** (`api.bcb.gov.br`) | Séries temporais do Banco Central | JSON | Diária | Selic meta e efetiva, CDI, IPCA, IGP-M, PTAX | Oficial e aberto. Séries por código (documentar os códigos em `bcb.py`). Desde 2025, série **diária** só com datas e em janelas de até 10 anos (senão HTTP 406) |
...
| **B3 — carteiras teóricas de índices** | Composição dos índices (Ibovespa, IFIX, IDIV, SMLL, IBRX 100…) e valor de fechamento | JSON do host de listagem da B3: `indexProxy/…/GetPortfolioDay` (carteira, com a data no cabeçalho) e `indexStatisticsProxy/…/GetPortfolioDay` (grade anual de fechamentos; a variação é calculada sobre o fechamento anterior) | Diária (carteira muda a cada quadrimestre, com prévias) | `indices`, `index_daily`, `index_compositions` | **Não documentado oficialmente**; mesma política de dados da B3 (§8.10). Fallback: manter a última carteira carregada e mostrar a data. Registrar em `data_sources` |
